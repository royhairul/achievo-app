...
class PesertaController extends Controller
{
    ...
    public function listPrestasi(Request $request)
    {
        ...
        $query = $prestasiPeserta = Prestasi::where('prestasi_peserta', $dataPeserta->peserta_id)
            ->with('Lomba') // Mengambil relasi lomba jika ada
        ...
        // Cek apakah peserta memiliki prestasi
        $hasPrestasi = $query->exists(); // Cek apakah ada prestasi peserta

        // Jika peserta belum mendaftar untuk lomba apapun
        if (!$hasPrestasi) {
            $noPrestasiMessage = 'Anda belum memiliki prestasi apapun.';
            $prestasi = collect(); // Set loma menjadi collection kosong
        } else {
            // Jika tidak ada keyword, tampilkan semua lomba yang diikuti peserta tanpa batasan waktu
            if (empty($keyword)) {
                $prestasi = $query->get(); // Ambil semua lomba tanpa batasan waktu
            } else {
                // Mencari berdasarkan kata kunci
                $keyword = strtolower($keyword);
                // Cek jika input adalah tahun
                if (preg_match('/^\d{4}$/', trim($keyword))) { // Memeriksa jika input adalah tahun (4 digit)
                    $year = trim($keyword);
                    $prestasi = $query->whereYear('prestasi_tanggal', $year)->get();
                } else {
                    // Mencari berdasarkan bulan dan tahun
                    if (preg_match('/^(january|february|march|april|may|june|july|august|september|october|november|december)(?:\s(\d{4}))?$/', $keyword, $matches)) {
                        $monthName = ucfirst($matches[1]); // Nama bulan
                        ...
                        // Mengatur query untuk mencari lomba di bulan dan tahun yang ditentukan
                        $prestasi = $query->whereMonth('prestasi_tanggal', $monthNumber)
                            ->whereYear('prestasi_tanggal', $year)
                            ->get();
                    } else {
                        // Jika ada keyword, lakukan pencarian berdasarkan nama lomba atau kategori
                        $prestasi = $query->where(function ($q) use ($keyword) {
                            $q->where('prestasi_nama', 'LIKE', "%$keyword%")
                                ->orWhere('prestasi_nominasi', 'LIKE', "%$keyword%")
                                ->orWhere('prestasi_kategori', 'LIKE', "%$keyword%")
                                ->orWhere('prestasi_penyelenggara', 'LIKE', "%$keyword%")
                                ;
                        })
                        ->get();
                    }
                ...
                if ($prestasi->isEmpty()) {
                    $noResultsMessage = 'Tidak ada yang cocok dengan pencarian ('.$keyword.')';
                    $prestasi = $query->get(); // Ambil semua lomba yang diikuti peserta
                }
            }
        }

        // Mengirim data peserta dan lomba yang diikuti ke view
        return view('peserta.list-prestasi', compact('dataPeserta', 'prestasi', 'noResultsMessage', 'noPrestasiMessage'));
    }
