...
@else
    <div class="wrapper mt-5 flex flex-col divide-y-2 max-h-64 overflow-y-auto"> <!-- Membatasi tampilan dan memungkinkan scroll -->
        @foreach ($prestasiPeserta->take(10) as $prestasi) <!-- Menampilkan maksimal 10 prestasi -->
        ...
        @endforeach
                        ...
                        <div class="mt-4 flex justify-center gap-2"> <!-- Ubah menjadi flex dengan justify-end untuk sejajar -->
            ...
            <div class="p-4 rounded bg-gray-100">
                ...
                @if($lombaDiikuti->isEmpty())
                    <p class="mt-4 text-center text-sky-500">Anda belum mendaftar untuk lomba manapun.</p>
                @else
                    <div class="wrapper mt-5 flex flex-col divide-y-2 max-h-64 overflow-y-auto"> <!-- Menambahkan max-height dan overflow -->
                        @foreach ($lombaDiikuti->take(10) as $lomba) <!-- Menampilkan maksimal 10 lomba -->
                            <div class="w-full flex items-center py-4 gap-x-2">
                                <p class="material-symbols-rounded text-sky-500 text-4xl">event</p>
                                <div class="grow">
                                    <h3 class="text-base font-bold">{{ $lomba->lomba_nama }}</h3>
                                    <p class="text-sm">Penyelenggara : {{ $lomba->penyelenggara->penyelenggara_nama }}</p>
                                    ...
                                    <p class="text-sm {{ $textColor }}">Batas Daftar &nbsp;&nbsp;&nbsp;&nbsp;: {{ $lombaDate->format('d F Y') }}</p>
                                </div>


                                <div class="justify-self-end relative">
                                    @if (\Carbon\Carbon::parse($lomba->lomba_tanggal)->isPast()) <!-- Cek apakah lomba_tanggal sudah lewat -->
                                        <a href="#" onmouseover="showTooltip(this)" onmouseout="hideTooltip(this)"
                        ...
                        <div class="mt-4 flex justify-center">
                            ...
                            <a href="{{ route('pesertaListLombaRoute') }}" class="flex w-max items-center px-4 py-2 gap-2 rounded-md bg-sky-500 text-sm font-medium text-white hover:bg-sky-600 focus:outline-none focus:ring focus:ring-sky-200 hover:shadow-lg shadow-sky-400 transition-all">
                                <span class="material-symbols-rounded">
                                    info
                                </span>
